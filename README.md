...
* разные способы написания звука /щ/ переключаются так же в конце слова, только с помощью **ww**/**"** (счёт/schiotw/schiot", щётка/schiotka, считаются/schitayutsjaww/schitayutsja"). Если есть неоднозначность, то отличие ставится явно префиксом **wsch**/**'sch** (считающейся/wschitayucsheysja/'schitayucsheysja).
