...
|    sch    |  None/qq  |  j/jqq   |   sch    | cs, cts, ckts, jy/iy |
...
**Слова на латинице из других языков** можно писать, экранируя с помощью ``" `"`` и ``"` "``. Этот вариант совместим с Markdown, т.к. перед конверсией Markdown будет конверсия в кириллицу и одинарные grave акценты исчезнут, а двойные - для кода - останутся.

Когда требуется пометить всего одно слово - до следующего знака препинания, то это можно сделать с помощью двух точек в начале слова. А если же нужно пометить кусок текста до следующего пробельного символа, то это можно сделать с помошью всего одной точки.
...
Для ситуаций когда требуется использовать русскую латиницу без конверсии в кириллицу придётся использовать другие разделители: ``" \`"`` и ``"\` "``.
