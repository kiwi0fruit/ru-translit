...
Слова на латинице из других языков можно писать, экранируя с помощью ``" `"`` и ``"` "`` (или ``"\w[(]?`" `` и `` "`[,.?;!)']?\w"``). Этот вариант совместим с Markdown, т.к. перед конверсией Markdown будет конверсия в кириллицу и одинарные grave акценты исчезнут, а двойные - для кода - останутся.

``On vypil butylcu `Coca-Cola` i zagovoril `Hello, do you speak English?` ``

Для ситуаций когда требуется использовать русскую латиницу без конверсии в кириллицу придётся использовать другие разделители.
